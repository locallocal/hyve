...
Hyve 需要记录智能体调用文本模型时由供应商返回的 Token 用量，并在以下位置展示：

- 项目右侧“智能体信息”面板：以每日图表展示当前项目 Token，支持按小时下钻；
- 智能体详情页面：展示该智能体所有历史项目的输入、输出和总 Token。
...
5. **删除语义明确**：清空项目只删除内容并保留用量；删除整个项目时才删除对应 Token 记录。
...
upsert 与 Token 记录同步在同一事务完成。清空项目仅删除 `messages`，保留
`token_usage_records`；删除整个项目则在同一事务删除两者。
...
项目内容区不直接展示 Token。用户点击工具栏的“智能体信息”后，在右侧停靠面板或抽屉内
查看当前项目统计：

- 智能体信息面板默认宽度为 360px，可在停靠模式下调整到 280–420px，并始终为项目正文保留最小可读宽度；
...
- “选择一天查看小时用量”提示复用通用设置中“项目执行状态”注释的字体样式，并与每日用量标题左对齐；
...
`ChatTokenUsageViewModel` 从 `MessageRepository` 加载当前项目的独立 Token 记录，在 UI 层
按本地时区进行日/小时分桶。它同时监听消息与项目 Repository 的变更流，因此新回复完成、
清空历史或删除操作会触发刷新；清空历史后的刷新仍读取保留的 Token 记录。每日视图的图标
摘要表示整个项目，小时视图的摘要只表示选中的一天。
...
- 清空项目：删除消息正文但保留 Token 事实记录，右侧面板刷新后统计不变；
- 删除项目：同时删除消息正文和该项目的 Token 事实记录；
