...
| GPT-Realtime 2.x | T/A/I → T/A | Realtime | 128K / 32K | 可配置推理、Functions、缓存、WebRTC/WebSocket/SIP | **不可用**：没有 Realtime 会话层 |
...
| **DeepSeek** (`deepseek`) | `deepseek-v4-flash`、`deepseek-v4-pro`；旧 `deepseek-chat`、`deepseek-reasoner` 已于 2026-07-24 退役，见[更新日志](https://api-docs.deepseek.com/updates/) | 内置 + 在线 | 两者均为 T、thinking、JSON、tool calls；只有 Flash 当前支持 Responses API。旧代码仍保留 `deepseek-reasoner` 判断，应删除；若要启用 MCP/自动 Skill，需实现结构化工具会话和工具结果回传，而非只读目录字段 |
...
| **Perplexity** (`perplexity`) | Search：`sonar`、`sonar-pro`；Reasoning：`sonar-reasoning-pro`；Research：`sonar-deep-research`，见[官方 Sonar 模型表](https://docs.perplexity.ai/docs/sonar/models) | 内置 + 在线 | 四个模型的 Web、thinking、deep research 元数据已覆盖；当前适配器没有结构化工具会话，MCP/自动 Skill 不可用。Agent API 的第三方模型预设不应混入 Sonar Chat 目录 |
