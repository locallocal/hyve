...
- 实现 `project.deliver_to_agent`；
- 实现交付目标、可见性、父子运行和循环限制；
- 实现交付卡片和审计详情；
- 覆盖超时、取消、重复交付和成员被移除场景。

### Phase 4：项目存储

- 创建 ProjectStorageRepository、ProjectArtifactRepository 和受控 Tool；
- 实现用户导入、新建、预览、搜索和版本历史；
- 实现 Agent 检索、分块读取、创建产物和写入新版本；
- 实现成员读写权限、路径隔离、原子写、内容索引和变更审计；
- 把需要持久化的附件和生成产物统一纳入 ProjectArtifact；
- 实现项目删除的文件暂存与恢复。
