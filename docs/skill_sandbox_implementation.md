...
本文档描述 Hyve Phase 5 中已经落地的桌面 Skill 脚本沙箱。内容以当前代码为准，
...
- 脚本进程不继承 Hyve 的环境变量、凭据、主目录或普通宿主文件系统；
...
状态被缓存，因此应用运行期间安装 helper 或修改内核策略后，需要重启 Hyve 才会
重新探测。
...
Hyve 的 API Key、代理设置、凭据路径、用户 Shell 配置和其他宿主环境变量不会自动传入。
...
超时或取消时，Hyve 向 `prlimit` 包装进程发送 `SIGKILL`。`bubblewrap` 使用
...
Hyve 不会在探测失败后尝试直接运行解释器。修复系统配置后应重启应用，再查看技能页的
沙箱状态。
